				...
				if (referenceBranch != null) {
					...
					if (_parent != null) {
						...
						if (IsFollowUp ()) {
							...
							positionFromBranch = _parent.curve.GetPointAt (1f).position - curve.GetPointAt(0f).position + _positionOffset;
							/*
							Vector3 parentPosition = _parent.curve.GetPointAt(1f).position;
						...
						// If not follow up.
							...
							//positionFromBranch = _parent.curve.GetPointAt(position).position + _positionOffset; 
						}
						positionFromRoot = _parent.positionFromRoot + positionFromBranch;
					}
					// If trunk.
					else {
						positionFromRoot = _positionOffset;
					}
				}
				for (int i = 0; i < _branches.Count; i++) {
