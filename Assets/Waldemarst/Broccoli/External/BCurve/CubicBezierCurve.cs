        ...
        */
        ...
        private void ApplyNoise (CurvePoint curvePoint) {
            ...
            Vector3 offset = new Vector3 (
                Mathf.PerlinNoise (
                    (curvePoint.lengthPosition + noiseLengthOffset + 0.5f) * Mathf.Lerp (noiseScaleAtFirstNode, noiseScaleAtLastNode, curvePoint.relativePosition),
                    0) - 0.5f,
                0f,
                Mathf.PerlinNoise (
                    0, 
                    (curvePoint.lengthPosition + noiseLengthOffset) * Mathf.Lerp (noiseScaleAtFirstNode, noiseScaleAtLastNode, curvePoint.relativePosition)) - 0.5f
            );
            */
            float noiseResolution = 4f;
            float noiseSmoothness = 0.2f;
            float pointNoiseX = (curvePoint.lengthPosition + noiseLengthOffset + 0.5f) * Mathf.Lerp (noiseScaleAtFirstNode, noiseScaleAtLastNode, curvePoint.relativePosition);
            float pointNoiseY = (curvePoint.lengthPosition + noiseLengthOffset) * Mathf.Lerp (noiseScaleAtFirstNode, noiseScaleAtLastNode, curvePoint.relativePosition);
            Vector3 offset = new Vector3 (
                Mathf.PerlinNoise (pointNoiseX * noiseResolution, 0) * noiseSmoothness,
                0f,
                Mathf.PerlinNoise (0, pointNoiseY * noiseResolution) * noiseSmoothness
            );
